...
import { PageNumber, ButtonWrapper, PageHeader, ElementWrapper, PageMenu } from './styleLocal'
...
class PageList extends Component {
  ...
  render() {
      ...
      list = pages.map((page, index) => {
        if (page.tasks && index === pageNumber) {
          return (
            <div key={page._id}>
              <PageHeader>
                <PageNumber>{page.title[activeLanguage.value]}</PageNumber>
                <div>{page.description[activeLanguage.value]}</div>
                <ButtonWrapper>
    ...
    return (
      <>
        <PageMenu>
          <PageNav pages={pages} changePage={this.changePage} pageNumber={pageNumber} activeLanguage={activeLanguage} />
        </PageMenu>
